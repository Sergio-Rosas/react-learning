...
class App extends React.Component {
    state = {
        location: "medellin", isLoading: false, displayLocation: "", weather: {},
    };
    ...
    render() {
        return (<div className="app">
            ...
            <Input location={this.state.location} onSetLocation={this.setLocation}/>
...
class Weather extends React.Component {
    ...
    render() {
        const {
            temperature_2m_max: max, temperature_2m_min: min, time: dates, weathercode: codes,
        } = this.props.weather;

        return (<div>
            <h2>Weather {this.props.location}</h2>
            <ul className="weather">
                {dates.map((date, index) => <Day date={date} max={max.at(index)} min={min.at(index)}
                                                 code={codes.at(index)} isToday={index === 0} key={date}/>)}
            </ul>
        </div>);
    }
}
